...
class LoginSignup extends React.Component {
	...
	handleLoginSubmit = () => {
		...
		let feedback = '';
		if (!email.length) feedback += 'Email field is empty. ';
		if (!password.length) feedback += 'Password field is empty. ';
		if (email.length > 0 && password.length > 0) {
			// Try logging in
			// Save feedback from backend and update
			alert('Thanks ' + email + ', your pass is ' + password);
		}
		loginDetails.feedback = feedback;
		this.setState({ loginDetails });
	};
				...
				<Card className="login-card">
					...
					<Card.Body className="login-body">
						<Form>
							...
							<FormFeedback>{loginDetails.feedback}</FormFeedback>
						</Form>
					</Card.Body>
